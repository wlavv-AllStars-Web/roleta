...
// Vérifie que l'utilisateur peut modifier ce lot
$stmt = $pdo->prepare("SELECT * FROM roleta_prizes WHERE id = ? AND user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($newStock <= 0) {
        ...
        $stmt = $pdo->prepare("UPDATE roleta_prizes SET stock = ? WHERE id = ? AND user_id = ?");
        $stmt->execute([$newStock, $id, $user_id]);
...
<div class="login-container">
    ...
    <form method="post" action="modifier_prix.php?id=<?= $id ?>&lang=<?= $lang ?>">
        <label><?= __menu('prize_name') ?>: <strong><?= htmlspecialchars($lot['name']) ?></strong></label><br><br>
